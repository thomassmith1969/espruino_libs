...
function RestCall(path, func, mime) {
  ...
  this.handleRequest = ((request, response, contentType) => {
    ...
    try {
      ...
      if (this.request.method == "POST" && contentType == "application/json") {
        var receivedData = request.read();
        try {
          const parsed = JSON.parse(receivedData);
          this.request["json"] = parsed;
          params.push(parsed);
        } catch (parseError) {}
      }
      var proc = undefined;
      ...
      if (proc instanceof StorageFile) {
        ...
        var index = 0; //chunkSize;
        var writing = true;
        // const firstChunk=storage.read(proc.name,0,index);//read the first chunk
        response.on("drain", () => {
          if (writing) {
...
module.exports = function (path, func, mime) {
  const restCall = new RestCall(path, func, mime ? mime : "application/json");
